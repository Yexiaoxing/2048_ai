...
import { type GameResult, type GameTrace, ResultsLogger, TraceLogger } from "./logger";
...
function loadConfig(configPath?: string): CLIConfig {
    ...
    console.log(chalk.gray(`Loading config from: ${resolvedPath || "default settings"}`));
...
async function main() {
    try {
        ...
        console.log(chalk.gray(`  Models: ${config.models.map((m) => m.name).join(", ")}`));
        console.log(chalk.gray(`  Runs per model: ${config.numRuns}`));
        console.log(chalk.gray(`  Visualization: ${config.visualize ? "enabled" : "disabled"}`));
        ...
        for (let modelIdx = 0; modelIdx < config.models.length; modelIdx++) {
            ...
            try {
                ...
                if (stats && !("error" in stats)) {
                    ...
                    console.log(chalk.cyan("\n📊 Summary Statistics:"));
                    console.log(
                        chalk.gray(`  Successful runs: ${stats.successfulRuns}/${stats.totalRuns}`),
                    );
                    console.log(
                        chalk.gray(
        ...
        for (const [name, stats] of Object.entries(modelSummary)) {
            ...
            if ("error" in stats) {
                ...
                console.log(
                    chalk.green(`    ✅ Successful: ${stats.successfulRuns}/${stats.totalRuns}`),
                );
