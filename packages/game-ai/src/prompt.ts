...
2. The obstacle tile (-2) is a wall and cannot be moved. Any numbers cannot pass it.
3. When two tiles with the same number collide while moving, they will merge into a tile with the total value of the two tiles that collided.
4. All tiles will slide as far as possible in the chosen direction until they are stopped by either another tile or the edge of the board. Note that tiles will only merge once per move.
5. If a move makes no changes to the board, penalty will be applied.
6. After each move, a new tile with a value of 2 or 4 will randomly appear in an empty spot on the board.
7. The game is won when a tile with the value of 2048 is created.
8. The game is lost when there are no valid moves left.
